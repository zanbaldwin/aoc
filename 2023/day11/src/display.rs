...
impl Display for Galaxy {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        let Galaxy { id, position: Position { x, y }, .. } = self;
...
impl Display for Universe {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        let galaxies: BTreeSet<Position> = self.galaxies.iter().map(|galaxy| galaxy.position).collect();
...
impl<'a> Display for Spacing<'a> {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        let mut result = String::with_capacity((self.universe.width + 2) * (self.universe.height + 1));
        ...
        universe.trim().lines().enumerate().for_each(|(index, line)| {
