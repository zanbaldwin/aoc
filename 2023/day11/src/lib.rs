    ...
    impl<'a> Spacing<'a> {
        pub(crate) fn expand(&self, exponential: usize) -> Universe {
            ...
            self.rows.iter().for_each(|row| {
                ...
                galaxies
                    .iter_mut()
                    .filter(|Galaxy { position: Position { x: _, y }, .. }| y > row)
                    .for_each(|Galaxy { shift: Position { x: _, y }, .. }| *y += growth);
            });
            self.columns.iter().for_each(|column| {
                width += growth;
                galaxies
                    .iter_mut()
                    .filter(|Galaxy { position: Position { x, y: _ }, .. }| x > column)
                    .for_each(|Galaxy { shift: Position { x, y: _ }, .. }| *x += growth);
            });

            galaxies.iter_mut().for_each(|galaxy| galaxy.resolve());

            Universe { width, height, galaxies }
        }
    }
    ...
    impl Universe {
        pub(crate) fn measure(&self) -> Spacing {
            Spacing {
                ...
                rows: (1..=self.height)
                    .filter(|line| {
                        self.galaxies.iter().filter(|Galaxy { position: Position { x: _, y }, .. }| y == line).count()
                            == 0
                    })
                    .collect(),
                columns: (1..=self.width)
                    .filter(|column| {
                        self.galaxies.iter().filter(|Galaxy { position: Position { x, y: _ }, .. }| x == column).count()
                            == 0
                    })
                    .collect(),
            }
        ...
        pub(crate) fn distances(&self) -> BTreeMap<(usize, usize), usize> {
            self.galaxies
                ...
                .map(|pairs| {
                    ...
                    ((first.id, second.id), first.position.distance_from(second.position))
                })
                .collect()
        }
    }
