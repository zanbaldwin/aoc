...
mod models {
    ...
    impl Race {
        ...
        pub fn get_beating_range(&self) -> Option<RangeInclusive<Time>> {
            ...
            let max = match (1..self.time).rev().find(|warmup: &Time| self.distance_after_warmup(*warmup)) {
                Some(max) => max,
...
mod parser {
    ...
    pub(crate) fn parse_race(input: &str) -> miette::Result<Race, AocError> {
        fn parse_kerning_numbers<'a>(list_name: &str) -> impl FnMut(&'a str) -> IResult<&'a str, u64> + '_ {
            move |input: &'a str| {
                map(preceded(tuple((tag(list_name), tag(":"), space1)), separated_list1(space1, digit1)), |digits| {
        ...
        let parser = map(
            separated_pair(parse_kerning_numbers("Time"), line_ending, parse_kerning_numbers("Distance")),
    ...
    pub(crate) fn parse_races(input: &str) -> miette::Result<Vec<Race>, AocError> {
        fn parse_number_list<'a>(list_name: &str) -> impl FnMut(&'a str) -> IResult<&'a str, Vec<u64>> + '_ {
            move |input: &'a str| {
                preceded(tuple((tag(list_name), tag(":"), space1)), separated_list1(space1, u64))(input)
            }
        }
